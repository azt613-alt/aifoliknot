...
# 8 הרשתות המרכזיות בחיבור שקיפות מחירים ממשלתי
SUPPORTED_CHAINS = [
...
def reset_database_with_1000_products(conn):
    with conn.cursor() as cur:
        ...
        # 4. מחירי בסיס ראשוניים לכל 1,000 המוצרים
        multipliers = {
...
def main():
    ...
    # איפוס והזנה של 1,000 מוצרים ו-544 סניפים
    reset_database_with_1000_products(conn)
    ...
    conn.close()
    logging.info(f"✨ סנכרון מלא הושלם! 1,000 מוצרים נטענו ו-{total_synced} מחירי אמת נמשכו מהשרתים.")
