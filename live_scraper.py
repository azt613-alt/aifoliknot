...
# מילון קואורדינטות ארצי לתרגום עיר/כתובת מתוך ה-XML ל-GPS
GEO_REGISTRY = {
...
def parse_stores_xml(file_url: str, auth, chain_id: str):
    """מפענח סניפים אך ורק מתוך קובץ ה-XML הרשמי"""
    stores = []
    try:
        ...
        logging.error(f"שגיאה בפענוח קובץ סניפים {file_url}: {e}")
...
def fetch_shufersal_stores_files():
    ...
    try:
        ...
        logging.warning(f"שופרסל: לא אותרו קבצי סניפים ({e})")
    return files
...
def sync_pure_official_stores(conn):
    """טוען אך ורק סניפים אמיתיים מתוך קובצי ה-XML הרשמיים שנפרקו בפועל"""
    logging.info("🏢 מושך ומפענח אך ורק קובצי Stores רשמיים משרתי הרשתות...")
    all_stores = []

    # 1. שופרסל
    for s_url in fetch_shufersal_stores_files():
        all_stores.extend(parse_stores_xml(s_url, None, "7290027600007"))
    ...
    for c_id, cfg in CHAIN_CONFIGS.items():
        ...
        for s_url in fetch_cerberus_stores_files(cfg["portal_url"], cfg["auth"]):
            all_stores.extend(parse_stores_xml(s_url, cfg["auth"], c_id))
    ...
    if unique_stores:
        with conn.cursor() as cur:
            ...
            conn.commit()
        logging.info(f"✨ נטענו {len(unique_stores)} סניפים רשמיים אמיתיים בלבד מה-XML!")
...
def main():
    ...
    # 1. טעינת סניפים אמיתיים בלבד מקובצי ה-XML הרשמיים
    sync_pure_official_stores(conn)

    # 2. הזנת קטלוג 1,000 מוצרים ומחירים לסניפים האמיתיים בלבד
    ...
    conn.close()
    logging.info(f"✨ סנכרון הושלם! כל הסניפים והמחירים הם סניפי אמת בלבד.")
